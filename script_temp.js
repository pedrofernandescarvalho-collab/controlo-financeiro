...
function sumExpenses() {
  return state.expenses
    .filter((expense) => getItemMonthKey(expense) === getMonthKey())
...
function sumFixedMonthlyExpenses() {
  ...
  const normalizedMonth = normalize(monthKey);
  
  return state.expenses
    .filter(e => e.kind === "fixed" && normalize(getItemMonthKey(e)) === normalizedMonth)
    .reduce((total, expense) => total + Number(expense.amount || 0), 0);
}
...
function sumIncomes() {
  return state.incomes
    .filter((income) => getItemMonthKey(income) === getMonthKey())
...
function calculateBudget() {
  ...
  // InteligÃªncia de RecuperaÃ§Ã£o: Se o salÃ¡rio estÃ¡ a zero (novo sistema), 
  // usamos o saldo inicial de dia 1 como base para nÃ£o quebrar a anÃ¡lise diÃ¡ria.
  const salary = Number(state.salary) || startBalance;
  
  const fixedExpenses = sumFixedMonthlyExpenses();
  const variableExpenses = sumVariableExpenses();
  const transferExpenses = sumTransfers();
  const extraIncomes = sumIncomes();
  
  const disposableMonthlyBudget = Math.max(salary + extraIncomes - fixedExpenses, 0);
  ...
  const leftover = Math.max(disposableMonthlyBudget - variableExpenses - transferExpenses, 0);
...
function sumExpensesUntil(day) {
  return state.expenses
    .filter((expense) =>
      getItemMonthKey(expense) === getMonthKey() &&
      expense.kind !== "fixed" &&
...
function sumFixedExpensesUntil(day) {
  return state.recurringFixed
    .filter((expense) => Number(expense.day) <= day)
    .reduce((total, expense) => total + Number(expense.amount || 0), 0);
}
...
function sumIncomesBetween(startDay, endDay) {
  return state.incomes
    .filter((income) =>
      getItemMonthKey(income) === getMonthKey() &&
      Number(income.day) > startDay &&
      Number(income.day) <= endDay
    )
    .reduce((total, income) => total + Number(income.amount || 0), 0);
}
...
function sumIncomesUntil(day) {
  return state.incomes
    .filter((income) =>
      getItemMonthKey(income) === getMonthKey() &&
      Number(income.day) <= day
    )
    .reduce((total, income) => total + Number(income.amount || 0), 0);
}
...
function getStartingSnapshot() {
  ...
  // Simplicidade total: O primeiro registo que o utilizador fizer no mes Ã© o "ponto de partida"
  return monthSnaps.length > 0 ? monthSnaps[0] : null;
}
...
function getCycleAnalysis() {
  ...
  const normalizedRevolutShare = shareTotal > 0 ? (Number(state.revolutShare) || 0) / shareTotal : 0.5;
  const today = getToday();
  const todayDayInActiveMonth = isActiveMonthCurrent() ? today.getDate() : (latestSnapshot ? latestSnapshot.day : 1);
  const elapsedDays = Math.min(todayDayInActiveMonth, daysInCycle);
  ...
  const expensesUntilToday = sumExpensesUntil(elapsedDays);
  const transfersUntilToday = sumTransfersUntil(elapsedDays);
  ...
  const incomesUntilSnapshot = sumIncomesUntil(snapshotDay);

  // O Total Real Gasto entre as entradas de saldos
  // = (Saldo Inicial - Saldo Atual) + Dinheiro Extra Recebido que esconde o que foi gasto
  ...
  // ATUALIZADO a pedido: A logica para o gasto efetivo (actualSpent) e o dinheiro que tinha - o que tem no dia (realFlexibleSpent)
  // Caso nÃ£o hajam snapshots, volta ao fallback das despesas escritas
  const actualSpent = hasProgressSnapshot
    ? realFlexibleSpentAtSnapshot
    : expensesUntilToday + transfersUntilToday;
...
function renderAnalysis() {
  ...
  document.querySelector("#analysisHint").textContent =
    `Ciclo atual: ${formatDate(analysis.cycleStart)} ate ${formatDate(analysis.cycleEnd)}. ` +
    `O gasto permitido ate ao dia ${analysis.latestDay} e ${formatCurrency(analysis.expectedSpentToday)}. ${balanceReferenceText}`;
  document.querySelector("#depositAdvice").textContent =
    `Se tiveres excedente, a sugestao indicativa e dividir ${formatCurrency(analysis.idealRemaining)}: ` +
    `a sugestao e ${formatCurrency(analysis.splitNowRevolut)} para Revolut e ` +
    `${formatCurrency(analysis.splitNowXtb)} para XTB.`;
...
function getReconciliationHistory() {
  ...
  for (let index = 1; index < snapshots.length; index += 1) {
    ...
    const incomeBetweenTotal = sumIncomesBetween(previousSnapshot.day, currentSnapshot.day);
...
function renderReceivables() {
  ...
  receivables.forEach((receivable) => {
    const node = template.content.firstElementChild.cloneNode(true);
    const statusLabel = receivable.status === "received" ? "Recebido" : "Por receber";
    ...
    editButton.addEventListener("click", () => {
      ...
      setStatus("#receivableStatusText", `A editar o valor em aberto de ${receivable.name}. Guarda para atualizar.`);
    });
...
function renderExpenses() {
  ...
  const allMonthExpenses = state.expenses
    .filter((expense) => normalize(getItemMonthKey(expense)) === normalizedMonth)
...
function renderRecurring() {
  const containers = [
    document.querySelector("#recurringList"),
    document.querySelector("#fixedExpensesList")
  ].filter(Boolean);
  ...
  containers.forEach(container => {
    container.innerHTML = "";
    if (!state.recurringFixed.length) {
      container.className = "item-list empty-state";
      container.textContent = "NÃ£o hÃ¡ despesas fixas globais configuradas.";
      return;
    }

    container.className = "item-list";
    const sortedFixed = state.recurringFixed.slice().sort((a, b) => a.day - b.day);

    sortedFixed.forEach((expense) => {
      const node = template.content.firstElementChild.cloneNode(true);
      node.querySelector(".item-title").textContent = expense.name;
      node.querySelector(".item-subtitle").textContent = `Assumida paga ao dia ${expense.day} todos os meses`;
      node.querySelector(".item-value").textContent = formatCurrency(expense.amount);
      
      node.querySelector(".ghost-btn").addEventListener("click", () => {
        state.recurringFixed = state.recurringFixed.filter((item) => item.id !== expense.id);
        saveState();
        render();
      });
      container.appendChild(node);
...
if (expenseForm) {
  ...
  if (!isCurrentMonthDate(expenseDate)) {
    setStatus("#expenseStatus", "A despesa tem de estar dentro do mes atual.");
    return;
  }
  ...
  if (kind === "fixed") {
     // Auditoria: Redirecionar despesas fixas para o novo sistema autÃ´nomo global
     ...
     setStatus("#expenseStatus", `Despesa ${name} registada como Fixa e integrada no fluxo automÃ¡tico mensal.`);
  } else {
