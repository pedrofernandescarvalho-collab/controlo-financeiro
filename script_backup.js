...
function getCycleAnalysis() {
  ...
  // Garantir ordem cronológica — getSnapshotsForMonth pode retornar entradas sem ordenação garantida
  const snapshots = getSnapshotsForMonth().slice().sort((a, b) => a.day - b.day);
  const startSnapshot = getStartingSnapshot();
  // latestSnapshot = o snapshot com o dia mais alto — sort explícito para garantir robustez
  const latestSnapshot = snapshots.length
    ? snapshots.slice().sort((a, b) => a.day - b.day)[snapshots.length - 1]
    : null;
  const shareTotal = (Number(state.revolutShare) || 0) + (Number(state.xtbShare) || 0);
  const normalizedRevolutShare = shareTotal > 0 ? (Number(state.revolutShare) || 0) / shareTotal : 0.5;
  const today = getToday();
  ...
  const incomesUntilSnapshot = sumIncomesUntil(snapshotDay, true); // Excluir reembolsos para a análise de gasto REAL
  ...
  // Caso não hajam snapshots, volta ao fallback das despesas escritas
...
function renderRecurring() {
  ...
  if (monthlyContainer) {
    ...
    const relevantForThisMonth = state.recurringFixed.filter(rf => {
      if (!rf.frequency || rf.frequency === 'monthly') return true;
    ...
    if (relevantForThisMonth.length === 0) {
      monthlyContainer.className = "item-list empty-state";
      monthlyContainer.textContent = "Não existem pagamentos fixos agendados para este mês.";
    } else {
      monthlyContainer.className = "item-list";
      relevantForThisMonth.forEach((item) => {
        const node = template.content.cloneNode(true);
        node.querySelector(".item-title").textContent = item.name;
        node.querySelector(".item-subtitle").textContent = `Pagamento fixo (${freqLabels[item.frequency] || "Mensal"}) previsto para dia ${item.day}`;
        node.querySelector(".item-value").textContent = formatCurrency(item.amount);
        node.querySelector(".ghost-btn").style.display = "none"; // Não remover daqui, remover na configuração
...
function renderRecurring() {
  ...
  containers.forEach(container => {
    ...
    if (!state.recurringFixed.length) {
      container.className = "item-list empty-state";
      container.textContent = "Não há despesas fixas globais configuradas.";
      return;
    }
